...
public ResponseEntity<Recipe> createRecipe(@RequestBody Recipe recipe) {
  try {
    Recipe _recipe = recipeRepository.save(new Recipe(recipe.getName(),recipe.getAuthorId(),recipe.getTime(),recipe.getComplexity(),recipe.getDescription(),recipe.getIngredients(),recipe.getSteps(),recipe.getTags(),recipe.getServes(), recipe.getPictureId()));
...
public ResponseEntity<Recipe> updateRecipe(@PathVariable("id") String id, @RequestBody Recipe recipe) {
  ...
  if (recipeData.isPresent()) {
    ...
    _recipe.setAuthor(recipe.getAuthorId());
    _recipe.setTime(recipe.getTime());
    _recipe.setComplexity(recipe.getComplexity());
